...
if (isset($_POST["update_user"])){
    ...
    $result = updateUser($id, $username, $password, $email, $role, $status);
    if ($result){
        redirect('users.php');
    }
}
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Update user  №<?=$result["id"]?></h3>
                            </div>
                            <form method="post" id="quickForm" data-toggle="validator"> <!--role="form" novalidate="novalidate"-->
                                <div class="card-body col-sm-6">
                                    <div class="form-group has-feedback">
                                        <label for="firstName">Username</label>
                                        <input class="form-control" name="username" id="username" type="text" value="<?=$result["username"]?>" required>
                                        <div class="help-block with-errors"></div>
                                    </div>
                                    <div class="form-group has-feedback">
                                        <label for="firstName">Password</label>
                                        <input class="form-control" name="password" id="password" type="password"  value="<?=$result["password"]?>" required>
                                        <div class="help-block with-errors"></div>
                                    </div>
                                    <div class="form-group has-feedback">
                                    ...
                                    <div class="form-group has-feedback col-sm-6">
                                        ...
                                        <select class="form-control" name="role" id="role">
                                            <?php foreach (getDataBYtable("users") as $item): ?>
                                                <option value="<?=$item['role']?>"
                                                    <?php echo (isset($_GET['id']) && $item['role'] == $result['role']) ? 'selected' : ""; ?>><?=$item['role']?></option>
                                            <?php endforeach; ?>
                                        </select>
                                        <div class="help-block with-errors"></div>
                                    </div>
                                        ...
                                        <select class="form-control" name="status" id="status">
                                            <?php foreach (getDataBYtable("users") as $item): ?>
                                                <option value="<?=$item['status']?>"
                                                    <?php echo (isset($_GET['id']) && $item['status'] == $result['status']) ? 'selected' : ""; ?>>
                                                    <?=($item['status'] == 9) ? 'INACTIVE': 'ACTIVE';?>
                                                </option>
                                            <?php endforeach; ?>
                                        </select>
                                        <div class="help-block with-errors"></div>
                                ...
                                <div class="card-footer">
                                    <button type="submit" class="btn btn-success" name="update_user">Обновить</button>
                                </div>
                            </form>
                        </div>
