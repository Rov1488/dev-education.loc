...
if (isset($_POST["update_day"])){
    ...
    $result = updateGroupDay($id, $dayName, $group_id);
    if ($result){
                    ...
                    <div class="col-md-12">
                        <div class="card card-primary">
                            <div class="card-header">
                                <h3 class="card-title">Update group day №<?=$result["id"]?></h3>
                            </div>
                            <form method="post" id="quickForm" data-toggle="validator"> <!--role="form" novalidate="novalidate"-->
                                <div class="card-body">
                                    <div class="form-group has-feedback">
                                        ...
                                        <input class="form-control" name="dayName" id="dayName" type="text" value="<?=$result["day_name"]?>" required>
                                        ...
                                        <label for="lastName">Group number</label>
                                            <select class="form-control" name="group_id" id="group_id">
                                                ...
                                                <option value="<?=$item['id']?>"
                                                    <?php echo (isset($_GET['id']) && $item['id'] == $result['group_id']) ? 'selected' : ""; ?>><?=$item['groupName']?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        <div class="help-block with-errors"></div>
                                    </div>
                                <div class="card-footer">
                                    <button type="submit" class="btn btn-success" name="update_day">Обновить</button>
                                </div>
                            </form>
                        </div>
